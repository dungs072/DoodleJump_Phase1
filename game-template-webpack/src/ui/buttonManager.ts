...
class ButtonManager{
    ...
    public onClick(position: Vector2){
        this.buttons.forEach(button=>{
            if(position.x>button.getPosition().x && position.x < button.getPosition().x + button.getScale().x &&
                position.y>button.getPosition().y && position.y<button.getPosition().y + button.getScale().y){
                    button.onClick();
                }
        })
    }
