...
class Button extends UIElement{
    ...
    constructor(position: Vector2, scale: Vector2, text: string, textColor: string, background: Sprite){
    ...
    public draw(context: CanvasRenderingContext2D): void {
        ...
        context.fillText(this.text, this.getPosition().x + this.getScale().x/2, this.getPosition().y + this.getScale().y/2);
        context.strokeStyle = '#008000';
        context.strokeRect(this.getPosition().x,
                        this.getPosition().y,
                        this.getScale().x,
                        this.getScale().y)
    }
